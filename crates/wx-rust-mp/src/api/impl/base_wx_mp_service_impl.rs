//! 基础实现（对应 Java `me.chanjar.weixin.mp.api.impl.BaseWxMpServiceImpl`）。
//!
//! Java 的 `BaseWxMpServiceImpl` 承载执行引擎（指数退避重试 + token 自动
//! 单次刷新）与通用 API。Rust 中 trait 无法携带泛型方法（破坏 dyn 兼容），
//! 故将执行引擎抽为本模块的泛型自由函数，由门面 trait 的 `get`/`post`
//! 默认实现调用——同一语义、同一文件映射。
//!
...
//!   （mp 侧无 host 替换——域名差异由 URL 构造层
//!   （`WxMpHostConfig.api_host`）承担，非执行期替换）。
//!
//! 两条路径共用 [`retry_loop`]（-1 系统繁忙指数退避重试，语义自原
//! `execute_with_retry` 原样抽出）；mp 侧差异点（token 失效
//! 「加锁比对→置过期」闭包、`auto_refresh_token` 重放开关）保留在本模块。
...
/// 执行请求（对应 Java `BaseWxMpServiceImpl.execute`）。
///
...
/// GET 门面经统一管线的执行入口（供 `WxMpService::get` 委托，对应 Java
...
/// POST 门面经统一管线的执行入口（供 `WxMpService::post` 委托，对应 Java
...
/// - mp 无执行期 host 替换（自定义域名由 URL 构造层的
///   `WxMpHostConfig.api_host` 承担，与 miniapp 的 `effective_api_host_url`
///   执行期替换不同）；
