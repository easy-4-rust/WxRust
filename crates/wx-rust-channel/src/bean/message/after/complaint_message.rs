//! 纠纷消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.after.ComplaintMessage.java`
//! （继承 `WxChannelMessage`）。

use serde::{Deserialize, Serialize};

use crate::bean::message::after::ComplaintInfo;

/// 纠纷消息（对应 Java `ComplaintMessage`）。
...
pub struct ComplaintMessage {
    ...
    /// 状态信息（对应 Java `info`）。
    #[serde(rename = "finder_shop_complaint", default)]
    pub info: Option<ComplaintInfo>,
}
