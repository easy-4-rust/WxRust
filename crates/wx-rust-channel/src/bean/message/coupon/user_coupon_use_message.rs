//! 用户卡券使用 消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.coupon.UserCouponUseMessage.java`
//! （继承 `WxChannelMessage`）。注意：Java 字段 `userCouponInfo` 的 JSON/XML
//! 属性名为 `user_info`（非 `user_coupon_info`），照抄。

use serde::{Deserialize, Serialize};

use crate::bean::message::coupon::UserCouponActionInfo;

/// 用户卡券使用 消息（对应 Java `UserCouponUseMessage`）。
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct UserCouponUseMessage {
