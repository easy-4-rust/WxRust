//! 小店修改名称消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.store.NicknameUpdateMessage.java`
//! （继承 `WxChannelMessage`）。

use serde::{Deserialize, Serialize};

/// 小店修改名称消息（对应 Java `NicknameUpdateMessage`）。
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct NicknameUpdateMessage {
    ...
    /// 小店旧昵称（对应 Java `oldNickname`）。
    #[serde(rename = "old_nickname", default)]
    pub old_nickname: Option<String>,
    /// 小店新昵称（对应 Java `newNickname`）。
    #[serde(rename = "new_nickname", default)]
