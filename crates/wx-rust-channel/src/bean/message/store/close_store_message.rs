//! 小店注销消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.store.CloseStoreMessage.java`
//! （继承 `WxChannelMessage`）。

use serde::{Deserialize, Serialize};

/// 小店注销消息（对应 Java `CloseStoreMessage`）。
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct CloseStoreMessage {
    ...
    /// Unix时间戳（秒，对应 Java `closeTimestamp`；Java 测试 golden 中为字符串，
    /// 数字/字符串均接受）。
    #[serde(
        rename = "close_timestamp",
        default,
        deserialize_with = "crate::bean::message::serde_helpers::opt_string_or_i64"
    )]
