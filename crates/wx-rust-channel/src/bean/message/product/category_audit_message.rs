//! 类目审核消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.product.CategoryAuditMessage.java`
//! （继承 `WxChannelMessage`）。
//!
//! Java 的 `ProductCategoryAudit` unpack setter 把嵌套对象字段合并到顶层字段；
//! Rust 以手动 `Deserialize` 表达同一合并语义（先顶层后嵌套、嵌套覆盖，ADAPTED）。
...
pub struct CategoryAuditMessage {
...
impl<'de> Deserialize<'de> for CategoryAuditMessage {
        ...
        D: Deserializer<'de>,
    {
        ...
        struct Raw {
            ...
            #[serde(rename = "ProductCategoryAudit")]
            product_category_audit: Option<HashMap<String, AnyScalar>>,
        }

        let raw = Raw::deserialize(deserializer)?;
        let mut msg = CategoryAuditMessage::default();
        ...
        if let Some(map) = raw.product_category_audit {
