//! 提现通知 消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.fund.WithdrawNotifyMessage.java`
//! （继承 `WxChannelMessage`）。

use serde::{Deserialize, Serialize};

use crate::bean::message::fund::WithdrawNotifyInfo;

/// 提现通知 消息（对应 Java `WithdrawNotifyMessage`）。
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct WithdrawNotifyMessage {
    ...
    /// 提现信息（对应 Java `withdrawInfo`）。
    #[serde(rename = "withdraw_info", default)]
    pub withdraw_info: Option<WithdrawNotifyInfo>,
}
