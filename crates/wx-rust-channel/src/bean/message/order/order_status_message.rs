//! 订单状态消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.order.OrderStatusMessage.java`
//! （继承 `WxChannelMessage`）。
//!
//! Java 的 `ProductOrderStatusUpdate` unpack setter 把嵌套对象字段合并到顶层
//! 字段；Rust 以手动 `Deserialize` 表达同一合并语义：先应用顶层字段，再应用
//! 嵌套对象（嵌套覆盖，ADAPTED：Jackson 按文档顺序后出现者覆盖；实际回调中
//! 两种形式同值时不可区分）。
...
/// 订单状态消息（对应 Java `OrderStatusMessage`）。
...
pub struct OrderStatusMessage {
    ...
    /// 订单状态（对应 Java `status`，见 `WxOrderStatus` 枚举）。
    #[serde(
        rename = "status",
        default,
...
impl<'de> Deserialize<'de> for OrderStatusMessage {
        ...
        D: Deserializer<'de>,
    {
        ...
        struct Raw {
            ...
            order_id: Option<AnyScalar>,
            #[serde(rename = "status")]
            status: Option<AnyScalar>,
            /// 嵌套对象（对应 Java unpack setter 的 `Map<String, Object>` 入参）。
            #[serde(rename = "ProductOrderStatusUpdate")]
            product_order_status_update: Option<HashMap<String, AnyScalar>>,
        }

        let raw = Raw::deserialize(deserializer)?;
        let mut msg = OrderStatusMessage::default();
        ...
        msg.order_id = raw.order_id.map(|v| v.0);
        msg.status = raw.status.and_then(|v| v.0.parse().ok());
        // 嵌套 unpack（对应 Java `unpackNameFromNestedObject`，后应用者覆盖）
        if let Some(map) = raw.product_order_status_update {
            if let Some(v) = map.get("order_id") {
                msg.order_id = Some(v.0.clone());
            }
