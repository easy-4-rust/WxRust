//! 订单取消消息。
//!
//! 对应 Java `me.chanjar.weixin.channel.bean.message.order.OrderCancelMessage.java`
//! （继承 `WxChannelMessage`）。

use serde::{Deserialize, Serialize};

use crate::bean::message::order::OrderCancelInfo;

/// 订单取消消息（对应 Java `OrderCancelMessage`）。
...
pub struct OrderCancelMessage {
    ...
    pub order_info: Option<OrderCancelInfo>,
}
