...
/// 获取商品实时库存（对应 Java `SPU_GET_STOCK_URL`）。
pub const GET_STOCK_URL: &str = "https://api.weixin.qq.com/channels/ec/product/stock/get";

/// 获取库存流水（对应 Java `SPU_GET_STOCK_FLOW_URL`）。
pub const GET_STOCK_FLOW_URL: &str = "https://api.weixin.qq.com/channels/ec/product/stock/getflow";

/// 批量获取实时库存（对应 Java `SPU_GET_STOCK_BATCH_URL`）。
pub const GET_STOCK_BATCH_URL: &str =
