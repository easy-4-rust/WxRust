...
fn dangling_weak() -> Weak<dyn WxPayService> {
    ...
    cfg.set_app_id("test").set_mch_id("test").set_mch_key("test");
...
async fn ecommerce_svc_error_applyment_id() {
    let svc = wx_rust_pay::api::r#impl::ecommerce_service_impl::EcommerceServiceImpl::new(dangling_weak());
...
async fn profit_sharing_svc_error() {
    let svc = wx_rust_pay::api::r#impl::profit_sharing_service_impl::ProfitSharingServiceImpl::new(dangling_weak());
