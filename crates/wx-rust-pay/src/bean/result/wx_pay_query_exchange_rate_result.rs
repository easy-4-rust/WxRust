//! 对应 Java `com.github.binarywang.wxpay.bean.result.WxPayQueryExchangeRateResult.java`。
//!
...
#[serde(rename = "xml")]
pub struct WxPayQueryExchangeRateResult {
    #[serde(
        default,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "rate_time")]
    pub rate_time: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "rate")]
    pub rate: Option<String>,
}

impl WxPayQueryExchangeRateResult {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        let mut v: Self = quick_xml::de::from_str(xml)
            .map_err(|e| format!("WxPayQueryExchangeRateResult 解析失败: {e}"))?;
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxPayQueryExchangeRateResult 序列化失败: {e}"))?;
