//! 对应 Java `com.github.binarywang.wxpay.bean.result.WxSignQueryResult.java`。
//!
...
pub struct WxSignQueryResult {
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "contract_state"
    )]
    pub contract_state: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "contract_signed_time"
    )]
    pub contract_signed_time: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "contract_terminated_time"
    )]
    pub contract_terminated_time: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "contract_termination_mode"
    )]
    pub contract_terminated_mode: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "contract_termination_remark"
    )]
...
impl WxSignQueryResult {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        let mut v: Self =
            quick_xml::de::from_str(xml).map_err(|e| format!("WxSignQueryResult 解析失败: {e}"))?;
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxSignQueryResult 序列化失败: {e}"))?;
