//! 对应 Java `com.github.binarywang.wxpay.bean.payscore.WxPartnerPayScoreRequest.java`。
//!
...
pub struct WxPartnerPayScoreRequest {
    #[serde(
        default,
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "result_notify_url"
    )]
    pub result_notify_url: Option<String>,
}

impl WxPartnerPayScoreRequest {
    /// 序列化为 JSON（对应 Java `toJson`）。
    pub fn to_json(&self) -> Result<String, String> {
        serde_json::to_string(self).map_err(|e| format!("WxPartnerPayScoreRequest 序列化失败: {e}"))
    }
}
