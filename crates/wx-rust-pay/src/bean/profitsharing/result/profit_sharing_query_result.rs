//! 对应 Java `com.github.binarywang.wxpay.bean.profitsharing.result.ProfitSharingQueryResult.java`。
//!
...
#[serde(rename = "xml")]
pub struct ProfitSharingQueryResult {
    #[serde(
        default,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "receivers")]
    pub receivers_json: Option<String>,
    /// 组合字段（线格式键与上文字段同名，serde(skip)，由 compose 逻辑组装）。
    #[serde(skip)]
...
pub struct Receiver {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "detailId")]
    pub detail_id: Option<String>,
}

impl ProfitSharingQueryResult {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        let mut v: Self = quick_xml::de::from_str(xml)
            .map_err(|e| format!("ProfitSharingQueryResult 解析失败: {e}"))?;
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("ProfitSharingQueryResult 序列化失败: {e}"))?;
