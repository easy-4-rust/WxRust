//! 对应 Java `com.github.binarywang.wxpay.bean.request.WxDepositUnfreezeRequest.java`。
//!
...
#[serde(rename = "xml")]
pub struct WxDepositUnfreezeRequest {
    ...
    pub unfreeze_fee: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "unfreeze_desc"
    )]
    pub unfreeze_desc: Option<String>,
}

impl WxDepositUnfreezeRequest {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        quick_xml::de::from_str(xml).map_err(|e| format!("WxDepositUnfreezeRequest 解析失败: {e}"))
    }
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxDepositUnfreezeRequest 序列化失败: {e}"))?;
