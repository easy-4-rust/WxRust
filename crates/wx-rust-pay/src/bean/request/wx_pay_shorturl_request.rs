//! 对应 Java `com.github.binarywang.wxpay.bean.request.WxPayShorturlRequest.java`。
//!
...
pub struct WxPayShorturlRequest {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "long_url")]
...
impl WxPayShorturlRequest {
    ...
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        quick_xml::de::from_str(xml).map_err(|e| format!("WxPayShorturlRequest 解析失败: {e}"))
    }
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxPayShorturlRequest 序列化失败: {e}"))?;
