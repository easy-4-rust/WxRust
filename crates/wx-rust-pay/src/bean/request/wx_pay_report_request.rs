//! 对应 Java `com.github.binarywang.wxpay.bean.request.WxPayReportRequest.java`。
//!
...
pub struct WxPayReportRequest {
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "interface_url"
    )]
    pub interface_url: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "execute_time_"
    )]
    pub execute_time: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "return_code"
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "user_ip")]
    pub user_ip: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "time")]
    pub time: Option<String>,
}

impl WxPayReportRequest {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        quick_xml::de::from_str(xml).map_err(|e| format!("WxPayReportRequest 解析失败: {e}"))
    }
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxPayReportRequest 序列化失败: {e}"))?;
