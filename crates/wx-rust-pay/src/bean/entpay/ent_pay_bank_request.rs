//! 对应 Java `com.github.binarywang.wxpay.bean.entpay.EntPayBankRequest.java`。
//!
...
pub struct EntPayBankRequest {
    ...
    pub enc_bank_no: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "enc_true_name"
    )]
    pub enc_true_name: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "bank_code")]
...
impl EntPayBankRequest {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        quick_xml::de::from_str(xml).map_err(|e| format!("EntPayBankRequest 解析失败: {e}"))
    }
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("EntPayBankRequest 序列化失败: {e}"))?;
