//! 对应 Java `com.github.binarywang.wxpay.bean.coupon.WxPayCouponInfoQueryRequest.java`。
//!
...
#[serde(rename = "xml")]
pub struct WxPayCouponInfoQueryRequest {
...
impl WxPayCouponInfoQueryRequest {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        quick_xml::de::from_str(xml)
            .map_err(|e| format!("WxPayCouponInfoQueryRequest 解析失败: {e}"))
    }
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxPayCouponInfoQueryRequest 序列化失败: {e}"))?;
