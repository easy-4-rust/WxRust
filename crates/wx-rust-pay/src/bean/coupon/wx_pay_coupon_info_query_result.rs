//! 对应 Java `com.github.binarywang.wxpay.bean.coupon.WxPayCouponInfoQueryResult.java`。
//!
...
#[serde(rename = "xml")]
pub struct WxPayCouponInfoQueryResult {
    #[serde(
        default,
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "coupon_use_value"
    )]
    pub coupon_use_value: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "coupon_remain_value"
    )]
    pub coupon_remain_value: Option<i32>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "begin_time"
    ...
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "consumer_mch_id"
    )]
    pub consumer_mch_id: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "send_source"
    )]
    pub send_source: Option<String>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
        rename = "is_partial_use"
    )]
    pub is_partial_use: Option<String>,
}

impl WxPayCouponInfoQueryResult {
    /// 从 XML 解析（对应 Java `fromXML`，XStream 语义：未知元素忽略、缺失字段默认）。
    pub fn from_xml(xml: &str) -> Result<Self, String> {
        let mut v: Self = quick_xml::de::from_str(xml)
            .map_err(|e| format!("WxPayCouponInfoQueryResult 解析失败: {e}"))?;
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        let out = quick_xml::se::to_string(self)
            .map_err(|e| format!("WxPayCouponInfoQueryResult 序列化失败: {e}"))?;
