//! 语音被动回复消息 builder。
//!
//! 对应 Java `me.chanjar.weixin.cp.bean.outxmlbuilder.VoiceBuilder`。

use crate::bean::message::WxCpXmlOutVoiceMessage;
...
impl VoiceBuilder {
    ...
    /// 构建消息（msgType 固定为 voice）。
    pub fn build(self) -> WxCpXmlOutVoiceMessage {
        let mut m = WxCpXmlOutVoiceMessage::new();
