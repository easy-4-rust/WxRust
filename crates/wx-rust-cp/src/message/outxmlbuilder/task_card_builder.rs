//! 任务卡片被动回复消息 builder。
//!
//! 对应 Java `me.chanjar.weixin.cp.bean.outxmlbuilder.TaskCardBuilder`。

use crate::bean::message::WxCpXmlOutTaskCardMessage;
...
impl TaskCardBuilder {
    ...
    /// 构建消息（msgType 固定为 update_taskcard）。
    pub fn build(self) -> WxCpXmlOutTaskCardMessage {
        let mut m = WxCpXmlOutTaskCardMessage::new();
