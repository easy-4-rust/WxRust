//! 企业微信消息路由器。
//!
//! 对应 Java `me.chanjar.weixin.cp.message.WxCpMessageRouter`：
...
//!    并追加等待任务统一收尾。
//!
//! 路由入口为 `WxCpXmlMessage`（对应 Java `route(WxCpXmlMessage)`），
//! 返回 `WxCpXmlOutMessage`；`WxCpMessage`（发送消息请求体）不参与路由。
...
use crate::message::{
    WxCpMessageHandler, WxCpMessageInterceptor, WxCpMessageMatcher, WxCpMessageRouterRule,
};
...
/// 企业微信消息路由器。
pub struct WxCpMessageRouter {
    /// 路由规则表（对应 Java `rules`）。
    rules: Vec<WxCpMessageRouterRule>,
    /// 企业微信服务（对应 Java `wxCpService`；测试场景可为空）。
    wx_cp_service: Option<Arc<dyn WxCpService>>,
...
impl WxCpMessageRouter {
    /// 新建消息路由器。
    ///
    /// # 参数
    /// - `wx_cp_service`：企业微信服务（可为 `None`，与 Java 测试
    ///   `new WxCpMessageRouter(null)` 的语义一致）
    ///
    /// 默认使用进程内单例内存去重器、`StandardSessionManager` 与
    /// `LogExceptionHandler`（对应 Java 默认构造的组件）。
    pub fn new(wx_cp_service: Option<Arc<dyn WxCpService>>) -> Self {
        Self {
            rules: Vec::new(),
            wx_cp_service,
    ...
    pub fn rules(&self) -> &[WxCpMessageRouterRule] {
        &self.rules
    }
    ...
    /// 处理微信消息（对应 Java `route(WxCpXmlMessage)`，默认空 context）。
    pub async fn route(&mut self, wx_message: &WxCpXmlMessage) -> Option<WxCpXmlOutMessage> {
        self.route_with_context(wx_message, &mut HashMap::new())
            .await
    }

    /// 处理微信消息（带上下文，对应 Java `route(WxCpXmlMessage, Map)`）。
    ///
    /// 1. 重复消息直接返回 `None`；
    ...
    pub async fn route_with_context(
        ...
        let mut match_rules: Vec<WxCpMessageRouterRule> = Vec::new();
        ...
        let service = self.wx_cp_service.clone();
        ...
        for rule in match_rules {
            if rule.is_async() {
                ...
                tokio::spawn(async move {
                    ...
                    // 异步操作结束，session 访问结束
                    session_manager.end_access(msg.from_user_name.as_deref().unwrap_or(""));
                });
                async_spawned = true;
            } else {
                // 同步执行：返回最后一个非异步规则的结果
                ...
                // 同步操作结束，session 访问结束
                session_manager.end_access(wx_message.from_user_name.as_deref().unwrap_or(""));
            }
        }
        ...
        if async_spawned {
            ...
            let session_manager = self.session_manager.clone();
            let from_user = wx_message.from_user_name.clone();
            tokio::spawn(async move {
    ...
    /// `createTime-agentId(trimToEmpty(String.valueOf))-fromUser-
    /// eventKey(trimToEmpty)-event(trimToEmpty)`；之后追加非空的
    /// `approvalInfo.spNo`、`userId`、`changeType`、`tagId`、`id`、
    /// `chatId`、`externalUserId`（对应 Java `append` 仅追加非空值）。
    /// Java `StringBuilder.append(null)` 追加字面量 `"null"`，Rust 侧对齐。
    pub fn is_msg_duplicated(&self, wx_message: &WxCpXmlMessage) -> bool {
        ...
        if wx_message.msg_id.is_none() {
            // String.valueOf(agentId)：null → "null"（trimToEmpty 不裁剪）
            let agent_id = wx_message
                .agent_id
                .clone()
                .unwrap_or_else(|| "null".to_string())
                .trim()
                .to_string();
            let event_key = wx_message.event_key.as_deref().unwrap_or("").trim();
            let event = wx_message.event.as_deref().unwrap_or("").trim();
            message_id.push_str(&format!(
                "{create_time}-{agent_id}-{from_user}-{event_key}-{event}"
            ));
        } else {
            message_id.push_str(&format!(
                "{}-{create_time}-{from_user}",
                wx_message.msg_id.as_deref().unwrap_or_default()
            ));
        }

        // 对应 Java `append(sb, value)`：仅追加非空值
        let append = |message_id: &mut String, value: Option<&str>| {
            if let Some(v) = value {
                if !v.is_empty() {
                    message_id.push('-');
                    message_id.push_str(v);
                }
            }
        };
        append(&mut message_id, wx_message.approval_info.sp_no.as_deref());
        append(&mut message_id, wx_message.user_id.as_deref());
        append(&mut message_id, wx_message.change_type.as_deref());
        append(&mut message_id, wx_message.tag_id.as_deref());
        append(&mut message_id, wx_message.id.as_deref());
        append(&mut message_id, wx_message.chat_id.as_deref());
        append(&mut message_id, wx_message.external_user_id.as_deref());

        self.message_duplicate_checker.is_duplicate(&message_id)
    }
}

/// 规则链式 builder（对应 Java `WxCpMessageRouterRule` 的链式配置方法）。
pub struct RuleBuilder<'a> {
    router: &'a mut WxCpMessageRouter,
    rule: WxCpMessageRouterRule,
}

impl<'a> RuleBuilder<'a> {
    /// 由 router 创建规则 builder。
    pub(crate) fn new(router: &'a mut WxCpMessageRouter) -> Self {
        Self {
            router,
            rule: WxCpMessageRouterRule::new(),
        }
    }
    ...
    /// 如果消息匹配某个 matcher（对应 Java `matcher(WxCpMessageMatcher)`，
    /// 用在用户需要自定义更复杂的匹配规则的时候）。
    pub fn matcher(mut self, matcher: Arc<dyn WxCpMessageMatcher>) -> Self {
        self.rule.matcher = Some(matcher);
        self
    }

    /// 设置微信消息拦截器（对应 Java `interceptor(WxCpMessageInterceptor)`；
    /// 多次调用依次追加，等价于 Java 变参版本）。
    pub fn interceptor(mut self, interceptor: Arc<dyn WxCpMessageInterceptor>) -> Self {
        self.rule.interceptors.push(interceptor);
        self
    }

    /// 设置微信消息处理器（对应 Java `handler(WxCpMessageHandler)`；
    /// 多次调用依次追加，等价于 Java 变参版本）。
    pub fn handler(mut self, handler: Arc<dyn WxCpMessageHandler>) -> Self {
        self.rule.handlers.push(handler);
        self
    }

    /// 规则结束：消息匹配该规则后将不再进入其他规则（对应 Java `end()`）。
    pub fn end(self) -> &'a mut WxCpMessageRouter {
        self.router.rules.push(self.rule);
        self.router
    }

    /// 规则结束，但是消息还会进入其他规则（对应 Java `next()`）。
    pub fn next(mut self) -> &'a mut WxCpMessageRouter {
