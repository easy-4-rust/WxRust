//! 对应 Java `me.chanjar.weixin.cp.bean.school.user.WxCpUserListResult.java`。
//!
...
pub struct Student {
    ...
    pub parents: Vec<crate::bean::school::user::wx_cp_user_list_result::Parent>,
}

impl WxCpUserListResult {
    /// 从 JSON 构建（对应 Java `fromJson`）。
    pub fn from_json(json: &str) -> Result<Self, String> {
        serde_json::from_str(json).map_err(|e| format!("WxCpUserListResult 解析失败: {e}"))
    }
}
...
impl WxCpUserListResult {
    ...
    pub fn to_json(&self) -> Result<String, String> {
        serde_json::to_string(self).map_err(|e| format!("WxCpUserListResult 序列化失败: {e}"))
    }
}
