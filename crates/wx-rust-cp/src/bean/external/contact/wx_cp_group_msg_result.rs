//! 对应 Java `me.chanjar.weixin.cp.bean.external.contact.WxCpGroupMsgResult.java`。
//!
...
impl WxCpGroupMsgResult {
    /// 从 JSON 构建（对应 Java `fromJson`）。
    pub fn from_json(json: &str) -> Result<Self, String> {
        serde_json::from_str(json).map_err(|e| format!("WxCpGroupMsgResult 解析失败: {e}"))
    }
}
