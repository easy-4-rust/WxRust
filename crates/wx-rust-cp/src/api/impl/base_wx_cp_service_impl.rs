...
//! - **SimpleGet/SimplePost 的带 token 流量**（门面 `get`/`post`）走
//!   [`wx_rust_common::pipeline::execute_pipeline`] 统一管线（见
//!   [`execute_get_via_pipeline`] / [`execute_post_via_pipeline`]）；
//! - **不带 token 通道（[`execute_normal`]）与通用执行器流量**
//!   （MediaUpload 等，经 [`execute_with_retry`] 进入）不走管线
//!   （管线恒注入普通 access_token，与专用 token / multipart 语义
//!   不同构），保留原路径。
//!
//! 两条重试路径共用 [`retry_loop`]（-1 系统繁忙指数退避重试，语义自原
//! `execute_with_retry` 原样抽出）；cp 侧差异点（企业微信 qyapi 域名由
//! URL 构造层承担、token 失效「加锁比对→置过期」闭包、
//! `auto_refresh_token` 重放开关）保留在本模块。
...
/// 「微信服务端异常，超出重试次数」收束（Java `WxRuntimeException` →
/// Rust `WxErrorException::from_code(-99, ...)`，ADAPTED）。
...
///
/// 通用执行器入口（保留供 MediaUpload 等任意 `RequestExecutor` 使用）；
/// SimpleGet/SimplePost 语义的 `get`/`post` 门面流量已改走
/// [`execute_get_via_pipeline`] / [`execute_post_via_pipeline`]。
...
/// GET 门面经统一管线的执行入口（供 `WxCpService::get` 委托，对应 Java
...
/// POST 门面经统一管线的执行入口（供 `WxCpService::post` 委托，对应 Java
...
/// SimpleGet/SimplePost 语义的带 token JSON errcode 流量）。
///
/// cp 侧差异点保留于本封装：
/// - uri 前置校验时序与 [`execute_internal`] 一致（拒绝携带 token 的
///   uri 发生在取 token 之前——拒绝路径零副作用；管线内部会再校验一次）；
/// - 企业微信 qyapi 域名无执行期替换（URL 构造层以
///   `WxCpConfigStorage` 域名体系承担，与 miniapp 执行期替换不同）；
...
///
/// 保留供通用执行器流量（MediaUpload 等，经 [`execute_with_retry`] 进入）；
